...
def _infer_nebula_root(start_file: Path) -> Path:
    """Infer the NEBULA repo root without hard-coding an absolute path.

    We walk upward from this script until we find:
        <root>/Configuration/NEBULA_PATH_CONFIG.py

    This matches the repository layout documented in Configuration/NEBULA_PATH_CONFIG.py.
    """
    here = start_file.resolve()
    for parent in here.parents:
        if (parent / "Configuration" / "NEBULA_PATH_CONFIG.py").is_file():
            return parent

    # Fallback (legacy): assume this file lives at
    #   <root>/Utility/ANIMATION/visualize_nebula_scene.py
    # so <root> is two directories up.
    return here.parents[2]


# Paths (portable; override-friendly)
NEBULA_ROOT = _infer_nebula_root(Path(__file__))
DEFAULT_NEBULA_OUTPUT_DIR = NEBULA_ROOT / "NEBULA_OUTPUT"

# Optional override for nonstandard layouts (e.g., if outputs live elsewhere).
# Example:
#   set NEBULA_OUTPUT_DIR=D:\data\nebula_outputs
NEBULA_OUTPUT_DIR = Path(os.environ.get("NEBULA_OUTPUT_DIR", str(DEFAULT_NEBULA_OUTPUT_DIR))).expanduser().resolve()

WINDOW_SOURCES_PKL = NEBULA_OUTPUT_DIR / "SCENE" / "obs_window_sources.pkl"
ZODIACAL_LIGHT_PKL = NEBULA_OUTPUT_DIR / "ZODIACAL_LIGHT" / "obs_zodiacal_light.pkl"
...
#
# The user request is to write here (relative to project root):
#   <NEBULA_ROOT>/NEBULA_OUTPUT/ANIMATIONS
ANIMATIONS_DIR = NEBULA_OUTPUT_DIR / "ANIMATIONS"
ANIMATIONS_DIR.mkdir(parents=True, exist_ok=True)

# Output filenames
POINTS_ANIM_OUT = ANIMATIONS_DIR / "nebula_points_unified.mp4"
GAUSS_ANIM_OUT = ANIMATIONS_DIR / "nebula_gauss_unified.mp4"


# Metadata placement
# - If True, the metadata box is drawn inside the image axes (over stars).
# - If False (default), the metadata box is drawn in the figure margin above the axes.
METADATA_IN_AXES = False

# When METADATA_IN_AXES is False, reserve figure space at the top so the metadata does not
# overlap the plotted image.
FIG_TOP_FOR_METADATA = 0.80
...
PSF_CACHE_STEP = 0.05  # pixels (e.g., 0.05 -> ~400 cached kernels)
...
def _safe_round_pix(x: np.ndarray, y: np.ndarray, cols: int, rows: int) -> Tuple[np.ndarray, np.ndarray]:
    """
    Round float pixel coordinates to nearest integer indices and clip to sensor bounds.
    """
    xi = np.clip(np.round(x).astype(np.int64), 0, cols - 1)
    yi = np.clip(np.round(y).astype(np.int64), 0, rows - 1)
    return xi, yi
...
def _save_with_tqdm(fig, update_func, n_frames: int, out_path: str | Path, fps: int, dpi: int, desc: str) -> None:
    writer = _build_writer(fps=fps)

    # Matplotlib's animation writers accept path-like objects in some versions,
    # but converting to str keeps behavior consistent across environments.
...
def main() -> None:
    ...
    # Exposure time (assume constant across window; taken from first star)
    any_star = next(iter(ws_win["stars"].values()))
    t_exp = float(np.asarray(any_star["t_exp_s"], dtype=np.float64)[0])
    ...
    omega_pix_sr_scalar = float(z_block.get("omega_pix", {}).get("omega_pix_sr_scalar", float("nan")))
    arcsec_per_pix = float("nan")
    if np.isfinite(omega_pix_sr_scalar) and omega_pix_sr_scalar > 0.0:
        arcsec_per_pix = _arcsec_per_pix_from_omega_pix_sr(omega_pix_sr_scalar)
        print(f"[INFO] omega_pix_sr_scalar = {omega_pix_sr_scalar:.6e} sr/pix -> ~{arcsec_per_pix:.2f} arcsec/pix")

    # Precompute u,v grid
    u, v = _normalized_uv_grids(rows, cols)
    ...
    bg_ref = _background_map_per_s(coeffs_per_s_pix[fi_ref], u, v, ZODI_MODEL) * t_exp  # ph/m^2/frame/pix
    ...
    # Compare stars to background at nearest pixel for a useful "visibility" metric
    xs0 = np.array([float(s["x_pix"][fi_ref]) for _, s in star_items], dtype=np.float64)
    ys0 = np.array([float(s["y_pix"][fi_ref]) for _, s in star_items], dtype=np.float64)
    sf0 = np.array([float(s["flux_ph_m2_frame"][fi_ref]) for _, s in star_items], dtype=np.float64)
    xi0, yi0 = _safe_round_pix(xs0, ys0, cols, rows)
    bg0_local = bg_ref[yi0, xi0]
    ratio0 = sf0 / np.maximum(bg0_local, 1e-30)

    print("\n[STATS] Stars vs background at nearest pixel (dimensionless ratio):")
    print(f"  median(star/bg_pix) = {float(np.median(ratio0)):.1f}")
    print(f"  5–95% range          = {float(np.percentile(ratio0,5)):.1f} .. {float(np.percentile(ratio0,95)):.1f}")
    print(f"  max(star/bg_pix)     = {float(np.max(ratio0)):.1f}")

    # Frame list to animate
    frame_idxs = np.arange(FRAME_START, n_frames, FRAME_STRIDE, dtype=np.int64)

    dt_frame_s = float(ws_track["dt_frame_s"])
    ...
    if SAVE_POINTS_ANIM:
        ...
        # If we place metadata outside the axes, reserve margin space at the top
        # so the metadata does not overlap the star field.
        if not METADATA_IN_AXES:
            fig1.subplots_adjust(top=FIG_TOP_FOR_METADATA)
        ...
        def render_points_frame(fi: int) -> np.ndarray:
            bg = _background_map_per_s(coeffs_per_s_pix[fi], u, v, ZODI_MODEL) * t_exp
            img = bg.copy()

            # Stars (vectorized deposit)
            xs = np.array([float(s["x_pix"][fi]) for _, s in star_items], dtype=np.float64)
            ys = np.array([float(s["y_pix"][fi]) for _, s in star_items], dtype=np.float64)
            sf = np.array([float(s["flux_ph_m2_frame"][fi]) for _, s in star_items], dtype=np.float64)
            ...
            for t in target_tracks:
                ...
                x, y, f = hit
                if POINTS_DEPOSIT_MODE == "nearest":
                    xi = int(np.clip(int(round(x)), 0, cols - 1))
                    yi = int(np.clip(int(round(y)), 0, rows - 1))
                    img[yi, xi] += f
                else:
                    _add_bilinear_splat(img, x, y, f)
        ...
        # Metadata (either in-axes or in the figure margin)
        if METADATA_IN_AXES:
            txt1 = ax1.text(
                0.01,
                0.99,
            ...
            # Figure-level text (figure coordinates) so it does not overlay stars.
            txt1 = fig1.text(
                0.01,
                0.99,
                "",
    ...
    if SAVE_GAUSS_ANIM:
        ...
        if not METADATA_IN_AXES:
            fig2.subplots_adjust(top=FIG_TOP_FOR_METADATA)
