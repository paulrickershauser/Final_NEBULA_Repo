...
- t_ref_utc is an ISO-8601 string in UTC (no leap seconds), derived from
  window start_time in the frames pickle (policy: reference time = start_time).
...
# Import Time to compute reference times and MJD.
...
class GaiaWindowQueryParams:
    ...
    t_ref_utc: Optional[str] = None
    t_ref_mjd_utc: Optional[float] = None
...
class GaiaWindowResult:
    ...
    t_ref_utc : str or None
        Reference time of the observation window in ISO-8601 UTC (policy: start_time).
    ...
    window_index: int
    t_ref_utc: Optional[str]
    t_ref_mjd_utc: Optional[float]
    sky_center_ra_deg: float
    sky_center_dec_deg: float
    sky_radius_deg: float
    query_radius_deg: float
...
def build_query_params_for_window(
    ...
    # Compute window reference epoch (policy: window start).
    t_ref_utc, t_ref_mjd_utc = _compute_window_ref_time(window, logger)
...
def _compute_window_ref_time(
    window: Dict[str, Any],
    logger: logging.Logger,
) -> Tuple[Optional[str], Optional[float]]:
    """
    Compute reference time of a window as ISO-UTC string and MJD(UTC).

    Current policy: reference time = window start_time.
    """
    start = window.get("start_time")
    if start is None:
        logger.debug(
            "NEBULA_QUERY_GAIA: window has no start_time; t_ref will be None."
        )
        return None, None

    try:
        t_ref = Time(start)
        return t_ref.utc.isot, float(t_ref.utc.mjd)
    except Exception as exc:
        logger.warning(
            "NEBULA_QUERY_GAIA: failed to parse reference time for window_index=%s: %r",
            window.get("window_index", -1),
            exc,
        )
        return None, None
...
def compress_gaia_table(
    ...
    result = GaiaWindowResult(
        ...
        t_ref_utc=params.t_ref_utc,
        t_ref_mjd_utc=params.t_ref_mjd_utc,
...
def build_gaia_cones_for_all_windows(
    ...
    Dict[str, Any]
        ...
        - "windows" list of GaiaWindowResult
          dictionaries (as produced by asdict()).
    ...
    for obs_name, obs_entry in obs_target_frames_with_sky.items():
        ...
        for window in tqdm(
            ...
            # Append the dataclass as a plain dictionary (shallow) to avoid
            # dataclasses.asdict() deep-copying large numpy arrays.
            window_results.append(dict(result.__dict__))
