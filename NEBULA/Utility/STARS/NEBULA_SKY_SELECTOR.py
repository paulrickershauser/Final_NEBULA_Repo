...
def attach_sky_footprints(
    ...
    safety_margin_deg: float | None = None,
    ...
    for obs_name, obs_entry in obs_target_frames.items():
        ...
        for w in windows:
            # Skip windows with no targets: do not annotate or write sky keys.
            ...
            # Define the footprint center at the window start epoch.
            center_idx = int(idx_range[0])
            center_ra_deg = float(ra_arr[center_idx])
            center_dec_deg = float(dec_arr[center_idx])
...
def main(logger: logging.Logger | None = None, force_recompute: bool = False) -> None:
    """
    Script entry point for NEBULA_SKY_SELECTOR.

    Steps
    -----
    1. Load ranked photon-only windows (with tracking_mode already attached):
    ...
    ranked_path = os.path.join(frames_dir, "obs_target_frames_ranked_with_tracking.pkl")
    if not os.path.exists(ranked_path):
        raise FileNotFoundError(
            f"Ranked target photon frames pickle not found at '{ranked_path}'. "
            "Run NEBULA_TRACKING_MODE.main() after TARGET_PHOTONS to generate it."
        )
    ...
    pointing_dir = os.path.join(NEBULA_OUTPUT_DIR, "POINT_SatPickles")
    obs_pointing_path = os.path.join(pointing_dir, "observer_tracks_with_pointing.pkl")

    if not os.path.exists(obs_pointing_path):
        raise FileNotFoundError(
            f"NEBULA_SKY_SELECTOR: missing prerequisite pointing pickle: '{obs_pointing_path}'. "
            "Run the POINTING stage first."
        )
