...
def attach_los_to_all_targets(
    *,
    ...
    target_tracks_with_los : dict
        ...
        their Track fields, including a per-observer mapping under:
            target['by_observer'][obs_name][<los_field>]
        where <los_field> corresponds to the configured field names
        (e.g. visibility_field="los_visible").
        For backwards compatibility, top-level LOS fields are only attached
        when there is exactly one observer; multi-observer runs store LOS
        exclusively under 'by_observer'.
    ...
    # In multi-observer runs, avoid writing top-level los_* fields on the target
    # track because they would be overwritten each time through the observer loop.
    # We only attach top-level fields when there is exactly one observer, to
    # preserve backwards-compatible single-observer behavior.
    attach_to_target = (len(observer_names) == 1)
    ...
    for obs_name, observer_track in observer_tracks.items():
        ...
        for idx, (tar_name, target_track) in enumerate(target_iter, start=1):
            ...
            # Call the LOS helper – compute LOS for this observer–target pair.
            # In multi-observer runs, we do NOT attach top-level los_* fields to
            # target_track (they would be overwritten on each loop iteration).
            # We only attach top-level fields when there is exactly one observer.
            result = NEBULA_VIS_PICKLE.attach_los_visibility_to_target(
                ...
                logger=logger,
                attach_to_target=attach_to_target,
            )
            ...
            obs_entry[visibility_field] = np.asarray(result["visible"], dtype=bool)
            obs_entry[h_field] = np.asarray(result["h"], dtype=float)
            obs_entry[regime_field] = np.asarray(result["regime"])
            obs_entry[fallback_field] = np.asarray(result["fallback"], dtype=bool)

            # Quick human-readable summary for this observer–target pair.
            frac_visible = float(obs_entry[visibility_field].mean())
