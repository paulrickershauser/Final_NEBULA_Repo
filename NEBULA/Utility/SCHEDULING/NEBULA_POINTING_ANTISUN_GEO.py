...
class PointingSchedule:
    ...
    boresight_roll_deg : np.ndarray
        ...
        shape (N,). (May be constant, but stored as a time series.)
...
def build_pointing_schedule_antisun_geo(
    ...
    # - If config.boresight_roll_deg is scalar: broadcast to (N,).
    # - If config.boresight_roll_deg is array-like: require length N.
    roll_input = getattr(config, "boresight_roll_deg", 0.0)
    roll_arr = np.asarray(roll_input, dtype=float)

    if roll_arr.ndim == 0:
        boresight_roll_deg = np.full(n_steps, float(roll_arr), dtype=float)
    else:
        boresight_roll_deg = roll_arr.reshape(-1)
        if boresight_roll_deg.size != n_steps:
            raise ValueError(
                f"boresight_roll_deg time series length {boresight_roll_deg.size} does not match times length {n_steps}"
            )
    # ------------------------------------------------------------------
    ...
    if store_on_observer:
        ...
        _set_track_field(observer_track, "pointing_boresight_roll_deg", boresight_roll_deg)
