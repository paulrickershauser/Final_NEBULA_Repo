...
5) Storing three unit conventions for the same underlying quantity:
   - per_pixel   : ph m^-2 s^-1 pix^-1   (derived from per_sr by multiplying by Ω_pix)
   - per_sr      : ph m^-2 s^-1 sr^-1    (backend output; resp_arrays["phi_ph_m2_s_sr"])
   - per_arcsec2 : ph m^-2 s^-1 arcsec^-2 (derived from per_sr via ARCSEC2_TO_SR)
...
def _times_to_utc_iso(times: Sequence[datetime]) -> List[str]:
    ...
    for i, t in enumerate(times):
        ...
        if t.tzinfo is None:
            raise ValueError(
                "Encountered tz-naive datetime; expected tz-aware UTC datetimes.\n"
                f"  offending index: times[{i}]\n"
                f"  offending value: {t!r}\n"
                "Fix: ensure upstream stages write timezone-aware UTC datetimes into "
                "obs_window_sources.pkl observer_geometry.state_vectors.times."
            )
...
def _run_wsl_for_radec_samples(
    *,
    ...
    Response arrays must contain:
        phi_ph_m2_s_sr : (n_frames, n_samples)
    with units ph m^-2 s^-1 sr^-1.
    ...
    # Fail-fast: verify the response unit contract.
    # This guards against a stale/mismatched WSL worker or backend that returns
    # a different normalization (e.g., per-pixel) while the Windows stage assumes
    # per-steradian (Option A).
    expected_schema = str(cfg.io.schema_version)
    if str(resp_meta.get("schema_version", "")) != expected_schema:
        raise ValueError(
            "WSL response schema_version mismatch.\n"
            f"  expected schema_version={expected_schema!r}\n"
            f"  got schema_version={resp_meta.get('schema_version', None)!r}\n"
            f"  response_base={resp_base}"
        )

    expected_quantity = "phi_ph_m2_s_sr"
    if str(resp_meta.get("quantity", "")) != expected_quantity:
        raise ValueError(
            "WSL response quantity mismatch.\n"
            f"  expected quantity={expected_quantity!r}\n"
            f"  got quantity={resp_meta.get('quantity', None)!r}\n"
            f"  response_base={resp_base}"
        )

    expected_units = "ph m-2 s-1 sr-1"
    if str(resp_meta.get("units", "")) != expected_units:
        raise ValueError(
            "WSL response units mismatch.\n"
            f"  expected units={expected_units!r}\n"
            f"  got units={resp_meta.get('units', None)!r}\n"
            f"  response_base={resp_base}"
        )

    if "phi_ph_m2_s_sr" not in resp_arrays:
        raise KeyError(
            "WSL response missing required array 'phi_ph_m2_s_sr'. "
            f"Available arrays: {sorted(resp_arrays.keys())}"
        )

    phi = np.asarray(resp_arrays["phi_ph_m2_s_sr"], dtype=np.float64)
    if phi.ndim != 2:
        raise ValueError(f"phi_ph_m2_s_sr must be 2D (n_frames,n_samples), got shape {phi.shape}")

    if phi.shape != (n_frames, n_samples):
        raise ValueError(
            "phi_ph_m2_s_sr shape mismatch.\n"
...
def build_window_zodi_product(
    *,
    ...
    phi_per_sr_samples = np.asarray(resp_arrays_s["phi_ph_m2_s_sr"], dtype=np.float64)  # (n_frames, n_samples)

    # Unit conversions (broadcast-safe).
    phi_per_pixel_samples = phi_per_sr_samples * omega_val
    ...
    if bool(cfg.field.export_map2d):
        ...
        if model_name is not None:
            ...
            phi_map_per_sr = np.asarray(resp_arrays_m["phi_ph_m2_s_sr"], dtype=np.float64)  # (n_frames, n_map_samples)
            phi_map_per_pixel = phi_map_per_sr * omega_map_val
