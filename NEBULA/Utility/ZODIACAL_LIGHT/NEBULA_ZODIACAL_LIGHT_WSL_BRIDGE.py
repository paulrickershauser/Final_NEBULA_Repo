...
def win_path_to_wslpath(path: Union[str, Path], *, distro: str) -> str:
    ...
    # IMPORTANT:
    # Invoke wslpath through bash and shell-quote the Windows path argument.
    #
    # Rationale:
    # Some WSL invocation paths effectively evaluate the command in a POSIX shell
    # context, where backslashes in unquoted Windows paths (e.g., "C:\Users\...")
    # can be treated as escape characters and stripped before wslpath sees them.
    #
    # Using shlex.quote() produces a single safe token (typically single-quoted),
    # preserving backslashes and whitespace so wslpath receives the exact string.
    bash_script = f"wslpath -a -u {shlex.quote(s)}"
    argv = _wsl_exe_prefix(distro) + ["bash", "-lc", bash_script]
    try:
