...
make_tan_wcs_from_pointing(boresight_ra_deg, boresight_dec_deg,
                           boresight_roll_deg, sensor_config) -> NebulaWCS
...
- The CD matrix is chosen such that:
      ...
      * A positive roll corresponds to a right-hand rotation of
...
# Import the NEBULA sensor configuration dataclass and default ACTIVE SENSOR instance.
...
def make_tan_wcs_from_pointing(
    ...
    This function constructs a simple celestial WCS with:
        ...
        - Orientation: RA-left and Dec-up at zero roll, with a focal-plane
          roll specified by `boresight_roll_deg`.
    ...
    boresight_roll_deg : float
        ...
        - boresight_roll_deg = 0 means the detector +y axis aligns with increasing
    ...
    - The CD matrix is built using a standard separation of scale and
      rotation:
          ...
          * rotation matrix based on boresight_roll_deg,
          * CD = diag(CDELT1, CDELT2) @ PC, where PC is a 2x2 rotation.

      This yields RA increasing to the left and Dec increasing up at
      zero roll, with boresight_roll_deg implementing an additional focal-plane
      rotation.
    """
    ...
    roll = float(boresight_roll_deg)

    # Obtain the horizontal pixel scale in degrees per pixel from the sensor configuration.
    # NOTE: SensorConfig.pixel_scale_deg is the integration contract for "how wide is a pixel?"
    # and is used to set the magnitude of the CDELT terms in this TAN WCS.
    ...
    # Build the 2x2 rotation matrix for the PC terms.
    #
    # IMPORTANT SIGN CONVENTION FIX:
    # Use the standard right-handed 2D rotation matrix:
    #     [[ cos(theta), -sin(theta)],
    #      [ sin(theta),  cos(theta)]]
    #
    # This matches Rachel's opticalSystem.define_proj_object:
    #     Frame_2_ICRS.pc = [[cos, -sin],
    #                        [sin,  cos]]
    #
    # and is consistent with the PointingConfig statement that "Positive roll
    # is a right-hand rotation about the boresight."
    pc11 = np.cos(theta)
    pc12 = -np.sin(theta)
    pc21 = np.sin(theta)
    pc22 = np.cos(theta)
...
def build_wcs_for_observer(
    ...
    In the current NEBULA schema, these pointing fields are **always**
    stored as arrays on the observer track (length N, matching
    observer_track.times), even when the pointing is constant in time.
    ...
    ------
    ValueError
        If the pointing fields are missing, are scalars, or have
        inconsistent lengths/shapes that prevent per-timestep WCS
        construction.
    """
    ...
    roll_field = _get_track_field(observer_track, "pointing_boresight_roll_deg")

    # Convert the RA, Dec, and roll fields to 1D float arrays.
    ra_arr = np.asarray(ra_field, dtype=float)
    dec_arr = np.asarray(dec_field, dtype=float)
    roll_arr = np.asarray(roll_field, dtype=float)

    # Enforce array-only pointing fields (no scalar roll/RA/Dec).
    if ra_arr.ndim == 0 or dec_arr.ndim == 0 or roll_arr.ndim == 0:
        raise ValueError(
            "build_wcs_for_observer: pointing fields must be arrays (length N) "
            "matching observer_track.times. Found scalar(s): "
            f"ra ndim={ra_arr.ndim}, dec ndim={dec_arr.ndim}, roll ndim={roll_arr.ndim}."
        )

    # Flatten to 1D while preserving ordering.
    ra_flat = ra_arr.reshape(-1)
    dec_flat = dec_arr.reshape(-1)
    roll_flat = roll_arr.reshape(-1)

    # Prefer validating against the observer timebase when available.
    times = _get_track_field(observer_track, "times", required=False)
    n_steps = len(times) if times is not None else None

    if n_steps is not None:
        if not (ra_flat.size == dec_flat.size == roll_flat.size == n_steps):
            raise ValueError(
                "build_wcs_for_observer: pointing field lengths must match "
                f"len(times)={n_steps}. Got ra={ra_flat.size}, dec={dec_flat.size}, "
                f"roll={roll_flat.size}."
            )
    else:
        # Fall back to internal consistency (times missing).
        if not (ra_flat.size == dec_flat.size == roll_flat.size):
            raise ValueError(
                "build_wcs_for_observer: inconsistent pointing field lengths on "
                "observer_track (times missing): "
                f"ra={ra_flat.size}, dec={dec_flat.size}, roll={roll_flat.size}."
            )
        n_steps = int(ra_flat.size)

    # If pointing is constant in time (stored as time series), return a single WCS.
    if (
        np.allclose(ra_flat, ra_flat[0])
        and np.allclose(dec_flat, dec_flat[0])
        and np.allclose(roll_flat, roll_flat[0])
    ):
        return make_tan_wcs_from_pointing(
            boresight_ra_deg=float(ra_flat[0]),
            boresight_dec_deg=float(dec_flat[0]),
            boresight_roll_deg=float(roll_flat[0]),
            sensor_config=sensor_config,
        )

    # Otherwise, build per-timestep WCS objects.
    wcs_list: List[NebulaWCS] = []
    for ra_val, dec_val, roll_val in zip(ra_flat, dec_flat, roll_flat):
        wcs_list.append(
            make_tan_wcs_from_pointing(
                boresight_ra_deg=float(ra_val),
                boresight_dec_deg=float(dec_val),
                boresight_roll_deg=float(roll_val),
                sensor_config=sensor_config,
            )
        )
