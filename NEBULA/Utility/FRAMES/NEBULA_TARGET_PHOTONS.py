...
def build_obs_target_frames_for_all_observers(
    max_frames_per_window: int | None = None,
    frames_pickle_path: str | None = None,
    logger: logging.Logger | None = None,
) -> Dict[str, Any]:
    """
    ...
    if frames_pickle_path is not None:
        # Treat relative paths as relative to NEBULA_OUTPUT_DIR (matches manifest usage).
        frames_path = Path(frames_pickle_path)
        if not frames_path.is_absolute():
            frames_path = NEBULA_OUTPUT_DIR / frames_path
    
        if not frames_path.exists():
            raise FileNotFoundError(
                f"NEBULA_TARGET_PHOTONS: frames_pickle_path '{frames_path}' not found. "
                f"Run the TARGET_FRAMES stage first."
            )
    
        logger.info("Loading photon frame catalog from '%s'.", frames_path)
        with open(frames_path, "rb") as f:
            frames_by_observer: Dict[str, Any] = pickle.load(f)

        if not isinstance(frames_by_observer, dict):
            raise TypeError(
                f"NEBULA_TARGET_PHOTONS: expected dict in frames pickle '{frames_path}', "
                f"got {type(frames_by_observer)!r}."
            )

    else:
        # Backward-compatible fallback (manual/script usage).
        logger.info(
    ...
    *,
    ...
    Steps:
        ...
        4. Cull & rank windows per observer and save a second pickle
           ...
           windows are removed.
    """
    global ranked_frames, obs_target_frames
    logger = get_logger()
    ...
    obs_target_frames = build_obs_target_frames_for_all_observers(
        max_frames_per_window=max_frames_per_window,
        frames_pickle_path=frames_pickle_path,
        logger=logger,
    )
