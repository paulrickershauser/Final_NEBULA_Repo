...
PHOTON_FRAMES_SUBDIR: str = "PHOTON_FRAMES"
PHOTON_FRAME_CATALOG_FILENAME: str = "obs_photon_frame_catalog.pkl"


def _default_photon_frame_catalog_path() -> Path:
    return (
        NEBULA_PATH_CONFIG.NEBULA_OUTPUT_DIR
        / PHOTON_FRAMES_SUBDIR
        / PHOTON_FRAME_CATALOG_FILENAME
    )


def save_obs_photon_frame_catalog_pickle(
    frames_by_observer: Dict[str, Any],
    *,
    ...
    out_path = Path(output_path) if output_path is not None else _default_photon_frame_catalog_path()

    # Resolve relative paths under NEBULA_OUTPUT_DIR for consistency with other stages.
    if not out_path.is_absolute():
        out_path = NEBULA_PATH_CONFIG.NEBULA_OUTPUT_DIR / out_path

    out_path.parent.mkdir(parents=True, exist_ok=True)

    with open(out_path, "wb") as f:
        pickle.dump(frames_by_observer, f)

    log.info("Photon frame builder: wrote frame catalog pickle -> '%s'.", out_path)
    return str(out_path)


def load_obs_photon_frame_catalog_pickle(
    *,
    input_path: Optional[str] = None,
    logger: Optional[logging.Logger] = None,
) -> Dict[str, Any]:
    log = logger or _get_logger()
    in_path = Path(input_path) if input_path is not None else _default_photon_frame_catalog_path()

    if not in_path.is_absolute():
        in_path = NEBULA_PATH_CONFIG.NEBULA_OUTPUT_DIR / in_path

    if not in_path.exists():
        raise FileNotFoundError(f"Photon frame builder: frame catalog pickle not found at '{in_path}'.")

    with open(in_path, "rb") as f:
        data = pickle.load(f)

    if not isinstance(data, dict):
        raise TypeError(f"Photon frame builder: expected dict in '{in_path}', got {type(data)!r}.")

    log.info("Photon frame builder: loaded frame catalog pickle <- '%s'.", in_path)
    return data
...
def main(
    *,
    ...
    Pipeline-stage entrypoint:
      - builds canonical photon frame grid (windows + frames[])
      - writes PHOTON_FRAMES/obs_photon_frame_catalog.pkl (by default)
    """
    log = logger or _get_logger()

    frames_by_observer = build_frames_by_observer_and_window_photon(
        max_frames_per_window=max_frames_per_window,
        force_recompute=force_recompute,
        logger=log,
    )

    save_obs_photon_frame_catalog_pickle(
        frames_by_observer,
