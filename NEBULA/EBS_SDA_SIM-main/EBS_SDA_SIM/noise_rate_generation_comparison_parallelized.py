...
def runDarkCurrentAnalysis(lock,I_dark):
    ...
    for i, file_i in enumerate(file_names):
        tic = time.time()
        circuit_test.dark_rate_analysis(I_dark, lock, num_pix_x, num_pix_y, num_steps_per_second, number_of_pulls, cuttoff_freq, pos_threshold, neg_threshold, seed, file_i)
        toc = time.time()
        ...
        print('Dark Current {} with a {} is done processing. Process took {} seconds.'.format(I_dark,test_style,(toc-tic)))
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(list(),columns=['Dark Current','Mean Event Rate','Standard Deviation','Events Per Pixel Per Second','Simulation Duration'])
    for i, file_i in enumerate(file_names):
        try:
    ...
    # Changing I_dark Sim
    # for specific list
    noise_rates = np.linspace(1e-15,5e-15,200)
    for i, noise in enumerate(noise_rates):
    	argsList.append((lock,noise*u.A))
    
    with mp.pool.Pool(processes=20,initializer=init_pool,initargs=(num_pix_x, num_pix_y, num_steps_per_second, number_of_pulls, cuttoff_freq, pos_threshold, neg_threshold, seed, file_names)) as pool:      
