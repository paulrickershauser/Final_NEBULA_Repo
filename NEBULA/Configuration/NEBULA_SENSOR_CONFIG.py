...
class SensorConfig:
    ...
    def pixel_scale_rad(self) -> float:
        ...
        However, when constructing a TAN (gnomonic) WCS, a more internally
        consistent "edge constraint" (used by Rachel's opticalSystem.define_proj_object)
        is:

            pixel_scale ≈ tan(fov_rad / 2) / (0.5 * (cols - 1))

        Notes:
        - This reduces to ~fov_rad / cols for small FOV, so it should not
          meaningfully disturb NEBULA at ~6–7 deg FOV (the difference is small).
        - Using (cols - 1) matches the FITS WCS convention of placing CRPIX at
          (cols + 1)/2 in 1-indexed pixel coordinates.
        """
        if self.cols <= 1:
            raise ValueError("pixel_scale_rad requires cols > 1.")
        return math.tan(self.fov_rad / 2.0) / (0.5 * (float(self.cols) - 1.0))
    ...
    def pixel_scale_deg(self) -> float:
        ...
        Historically this was `fov_deg / cols` (small-angle approximation).

        To align NEBULA's TAN WCS construction with Rachel's opticalSystem.define_proj_object
        (and the standard TAN/gnomonic scaling), we instead use:

            pixel_scale_deg = rad2deg(tan(deg2rad(fov_deg / 2))) / (0.5 * (cols - 1))

        This keeps the same property name so downstream code does not change,
        while making the WCS scale definition consistent across both simulators.
        """
        if self.cols <= 1:
            raise ValueError("pixel_scale_deg requires cols > 1.")
        return math.degrees(math.tan(self.fov_rad / 2.0)) / (0.5 * (float(self.cols) - 1.0))
...
GEN3_VGA_CD_SENSOR = SensorConfig(
    ...
    optical_throughput=1,           # placeholder; tune vs data
