...
# (Gaia G), rather than using the bolometric solar constant as a proxy.
#
# This implementation keeps NEBULA "pre-optics": it defines the *spectral window*
# used to interpret G-band magnitudes and reflectance, without applying any optics
# throughput or detector QE (those remain separate and can be applied later).
#
# Implementation note:
# - We model the solar SED as a 5772 K blackbody and compute the fraction of the
#   bolometric power in the nominal Gaia G support region.
# - For higher fidelity, you can replace this with an actual AM0 solar spectrum
#   and the true Gaia G response curve.
...
GAIA_G_IRRADIANCE_1AU_W_M2 = SOLAR_CONSTANT_BOL_W_M2 * GAIA_G_SOLAR_ENERGY_FRACTION_BB
'''end of new code'''
