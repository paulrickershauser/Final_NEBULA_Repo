...
- The pipeline treats zodiacal light as a band-integrated photon surface brightness / radiance:
    phi_ph_m2_s_sr  [ph m^-2 s^-1 sr^-1]
  This is instrument-independent and is the preferred “canonical” angular form.

Unit-flow contract (Option A)
-----------------------------
WSL backend output (native):
- The WSL backend returns the canonical per-steradian quantity:
    phi_ph_m2_s_sr  [ph m^-2 s^-1 sr^-1]

Windows-stage derived products:
- per_arcsec2:
    phi_ph_m2_s_arcsec2 = phi_ph_m2_s_sr * ARCSEC2_TO_SR
- per_pixel (requires Ω_pix from omega_pix.py):
...
(see Utility/ZODIACAL_LIGHT/omega_pix.py):
...
- Helper functions used by the Windows stage:
    resolve_window_sources_path(), resolve_tmp_dir(), resolve_output_path(), get_sensor_config(),
    convert_per_arcsec2_to_per_sr(), convert_per_sr_to_per_arcsec2(), convert_per_sr_to_per_pixel(),
    validate_config()
"""
...
class ZLUnitsConfig:
    ...
    store_native_per_sr:
        Store the WSL backend's native convention per steradian (per_sr).

        This is the canonical unit-flow policy:
        - WSL returns per_sr (instrument-independent)
        - Windows derives per_arcsec2 and per_pixel

    derived_outputs:
        Additional quantities to compute and store.
        - "per_arcsec2" : derived from per_sr via ARCSEC2_TO_SR
        - "per_pixel"   : derived from per_sr via Ω_pix (sr/pixel)

    Project requirement:
    - Store all three values (per_sr + per_arcsec2 + per_pixel).
    """
    store_native_per_sr: bool = True
    derived_outputs: Sequence[ZodiQuantity] = ("per_arcsec2", "per_pixel")
...
def validate_config(cfg: ZodiacalLightConfig = ZODIACAL_LIGHT_CONFIG) -> None:
    ...
    - unit-flow policy matches Option A (native per_sr; derived per_arcsec2 + per_pixel)
    - bandpass structural requirements
    - Ω_pix configuration sanity (shape policy; computation occurs in Windows stage)
    - if per_pixel requested and analytic mode, sensor geometry supports Ω_pix
    """
    # Field sampling
    ...
    # Units sanity (Option A)
    if not cfg.units.store_native_per_sr:
        raise RuntimeError(
            "units.store_native_per_sr must be True for the canonical unit policy "
            "(WSL backend returns per_sr)."
        )
    if "per_arcsec2" not in cfg.units.derived_outputs or "per_pixel" not in cfg.units.derived_outputs:
        raise RuntimeError(
            "units.derived_outputs must include both 'per_arcsec2' and 'per_pixel' "
    ...
    if bp.mode == "svo_id":
        ...
        if (not math.isfinite(c)) or (not math.isfinite(w)) or w <= 0.0:
            raise RuntimeError(
                f"Invalid tophat bandpass center/width: center_nm={bp.center_nm!r}, width_nm={bp.width_nm!r}"
            )
    else:
    ...
    # If per_pixel requested and analytic mode, verify sensor geometry supports Ω_pix now.
    #
    # We intentionally do not compute Ω_pix here (that happens in omega_pix.py); we only validate that
    # the required sensor fields exist and are well-formed so errors are immediate and actionable.
    if "per_pixel" in cfg.units.derived_outputs and cfg.omega_pix.mode == "analytic_scalar":
        p = float(ACTIVE_SENSOR.pixel_pitch)
        f = float(ACTIVE_SENSOR.focal_length)

        if not math.isfinite(p) or p <= 0.0:
            raise RuntimeError(f"sensor.pixel_pitch is invalid; expected finite > 0, got {ACTIVE_SENSOR.pixel_pitch!r}")
        if not math.isfinite(f) or f <= 0.0:
            raise RuntimeError(f"sensor.focal_length is invalid; expected finite > 0, got {ACTIVE_SENSOR.focal_length!r}")
