...
# tqdm: progress bar for MP4 encoding loops
# Use the console tqdm by default (Spyder/IPython friendly).
try:
...
NEBULA_OUTPUT_DIR = r"C:\Users\prick\Desktop\Research\NEBULA\NEBULA_OUTPUT"
...
# Output filenames (written next to this script by default)
POINTS_ANIM_OUT = "nebula_points_unified.mp4"
GAUSS_ANIM_OUT = "nebula_gauss_unified.mp4"
...
FFMPEG_CRF = 14
FFMPEG_PRESET = None  # Some ffmpeg builds reject '-preset'; set a string (e.g. "slow") only if supported.
...
def _build_writer(fps: int) -> FFMpegWriter:
    extra = [
        "-pix_fmt", str(FFMPEG_PIX_FMT),
    ]

    # Prefer CRF when available (quality knob independent of bitrate).
    # If your ffmpeg build rejects -crf, set FFMPEG_CRF=None and use FFMPEG_BITRATE instead.
    if FFMPEG_CRF is not None:
        extra += ["-crf", str(int(FFMPEG_CRF))]

    # Some ffmpeg builds reject '-preset'. Only include it when explicitly configured.
    if FFMPEG_PRESET:
